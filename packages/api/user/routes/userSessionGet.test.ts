...
describe.concurrent('GET /session', () => {
  ...
  describe<Context>('with invalid user session', (it) => {
    it('should return an error if the session is invalid', async({ setupUser: createUser, moduleUser, application }) => {
    ...
    it('should return an error if the session address is not matching', async({ setupUser: createUser, moduleUser, application }) => {
      const { session, headers } = await createUser()
      const { UserSession } = moduleUser.getRepositories()
    ...
    it('should return an error if the session user agent is not matching', async({ setupUser: createUser, moduleUser, application }) => {
      const { session, headers } = await createUser()
      const { UserSession } = moduleUser.getRepositories()
    ...
    it('should delete the local cookie if the session is invalid', async({ setupUser: createUser, moduleUser, application }) => {
      const { session, headers } = await createUser()
      const { UserSession } = moduleUser.getRepositories()
      session.expiresAt = new Date(0)
      await UserSession.save(session)
