...
describe.concurrent('PATCH /api/users/:username/enable', () => {
  ...
  describe<Context>('with super administrator', (it) => {
    it('should enable the user in the database', async({ setupUser, moduleUser, application }) => {
      ...
      const response = await application.fetch(`/api/users/${user.username}/enable`, { method: 'PATCH', headers })
  ...
  describe<Context>('with unauthenticated user', (it) => {
    it('should not enable the user in the database', async({ setupUser, moduleUser, application }) => {
      const { user } = await setupUser({ disabledAt: new Date() })
      const response = await application.fetch(`/api/users/${user.username}/enable`, { method: 'PATCH' })
      const data = await response.json() as Record<string, string>
