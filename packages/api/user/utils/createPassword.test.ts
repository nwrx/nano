/* eslint-disable @typescript-eslint/no-unsafe-assignment */
import type { Context } from '../../__fixtures__'
import { createContext } from '../../__fixtures__'
import { createPassword } from './createPassword'

describe.concurrent<Context>('createPassword', (it) => {
  beforeEach<Context>(async(context) => {
    context.ctx = await createContext()
    await context.ctx.createServer()
  })

  afterEach<Context>(async(context) => {
    await context.ctx.destroy()
  })

  it('should return the hash of the password', async({ expect, ctx }) => {
    const { hash } = await createPassword.call(ctx.ModuleUser, 'password')
    expect(hash).toBeTypeOf('string')
    expect(hash).toMatch(/^[\da-f]{1024}$/)
  })

  it('should return the options used to hash the password', async({ expect, ctx }) => {
    const { options } = await createPassword.call(ctx.ModuleUser, 'password')
    expect(options).toMatchObject({
      algorithm: 'scrypt',
      N: 16384,
      r: 8,
      p: 1,
      maxmem: 67108864,
      keylen: 512,
      encoding: 'hex',
      salt: expect.stringMatching(/^[\da-f]{64}$/),
    })
  })

  it('should return the hash of the password with the provided salt', async({ expect, ctx }) => {
    const result = await createPassword.call(ctx.ModuleUser, 'password', { salt: 'salt' })
    expect(result).toMatchObject({
      hash: '',
      options: expect.objectContaining({ salt: 'salt' }),
    })
  })

  it('should return the hash of the password with the provided key length', async({ expect, ctx }) => {
    const result = await createPassword.call(ctx.ModuleUser, 'password', { salt: 'salt', keylen: 8 })
    expect(result).toMatchObject({
      hash: '745731af4484f323',
      options: expect.objectContaining({ salt: 'salt' }),
    })
  })

  it('should assign the password to the user if provided', async({ expect, ctx }) => {
    const { User } = ctx.ModuleUser.getRepositories()
    const user = User.create({ username: 'username' })
    const result = await createPassword.call(ctx.ModuleUser, 'password', { user })
    expect(result.user).toStrictEqual(user)
  })

  it('should not generate the same hash for the same password', async({ expect, ctx }) => {
    const result1 = await createPassword.call(ctx.ModuleUser, 'password')
    const result2 = await createPassword.call(ctx.ModuleUser, 'password')
    expect(result1.hash).not.toStrictEqual(result2.hash)
  })
})
