...
describe('valueEncrypt', () => {
  const secret = 'test-secret-key'
  const value = 'test-value'

  it('should encrypt a value', async() => {
    const result = await valueEncrypt(value, secret, 'aes-256-gcm')
  ...
  it('should produce different ciphers for same value due to random IV', async() => {
    const result1 = await valueEncrypt(value, secret, 'aes-256-gcm')
    const result2 = await valueEncrypt(value, secret, 'aes-256-gcm')
  ...
  it.each(['aes-256-gcm', 'aes-192-gcm', 'aes-128-gcm'] as const)('should encrypt using %s', async(algorithm) => {
    const result = await valueEncrypt(value, secret, algorithm)
    expect(result.algorithm).toBe(algorithm)
  })

  it('should reject on invalid algorithm', async() => {
    // @ts-expect-error: invalid algorithm
    const shouldReject = valueEncrypt(value, secret, 'aes-256-cbc')
