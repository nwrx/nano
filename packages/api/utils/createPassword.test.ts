/* eslint-disable sonarjs/no-hardcoded-passwords */
/* eslint-disable @typescript-eslint/no-unsafe-assignment */
import { createPassword } from './createPassword'

describe.concurrent('createPassword', () => {
  describe('result', (it) => {
    it('should return the hash of the password', async() => {
      const result = await createPassword({ password: 'password' })
      expect(result.hash).toBeTypeOf('string')
      expect(result.hash).toMatch(/^[\da-f]{1024}$/)
    })

    it('should return the options used to hash the password', async() => {
      const options = await createPassword({ password: 'password' })
      expect(options).toMatchObject({
        algorithm: 'scrypt',
        N: 16384,
        r: 8,
        p: 1,
        maxmem: 67108864,
        keylen: 512,
        encoding: 'hex',
        salt: expect.stringMatching(/^[\da-f]{64}$/),
      })
    })

    it('should return the hash of the password with the provided salt', async() => {
      const result = await createPassword({ password: 'password', salt: 'salt' })
      expect(result).toMatchObject({
        hash: '',
        salt: 'salt',
      })
    })

    it('should return the hash of the password with the provided key length', async() => {
      const result = await createPassword({ password: 'password', salt: 'salt', keylen: 8 })
      expect(result).toMatchObject({
        hash: '745731af4484f323',
        salt: 'salt',
      })
    })

    it('should not generate the same hash for the same password', async() => {
      const result1 = await createPassword({ password: 'password' })
      const result2 = await createPassword({ password: 'password' })
      expect(result1.hash).not.toStrictEqual(result2.hash)
    })
  })
})
