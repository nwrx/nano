...
describe('askQuestion', () => {
  ...
  describe('question', () => {
    ...
    it('should resolve to the correct response when dispatching a "nodeQuestionResponse" event', async() => {
      ...
      thread.dispatch('nodeResponse', nodeId, { id: await eventId, response: 'Hello, World!' }, createEventMetadata(thread, node))
  ...
  describe('matching', () => {
    it('should not resolve when dispatching a "nodeQuestionResponse" for a different event', async() => {
      ...
      thread.dispatch('nodeResponse', nodeId, { id: 'differentId', response: 'response' }, createEventMetadata(thread, node))
    ...
    it('should not resolve when dispatching a "nodeQuestionResponse" for a different node', async() => {
      ...
      thread.dispatch('nodeResponse', nodeId2, { id: 'differentId', response: 'response' }, createEventMetadata(thread, node))
      vi.advanceTimersByTime(1)
      await expect(shouldReject).rejects.toThrow()
    })
  })
