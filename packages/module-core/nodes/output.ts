...
export const nodeOutput = defineNode({
  ...
  dataSchema: ({ data }: NodeInstanceContext) => defineDataSchema<OutputDataSchema>({
    type: {
  ...
  resultSchema: ({ data }: NodeInstanceContext) => {
    const { type } = data as OutputDataSchema
    return defineResultSchema({
      value: {
        type: { number, boolean, stream, text: string }[type ?? 'text'],
        name: 'Value',
        description: 'The value to send to the exitpoint.',
      },
    })
