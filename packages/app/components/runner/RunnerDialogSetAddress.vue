...
watch(isOpen, () => address.value = runner.data.address, { immediate: true })
...
en:
  ...
  text: Update the address where runner **{name}** can be reached.
  ...
  success: Runner **{name}** address updated successfully.
fr:
  title: Changer l'adresse du serveur d'exécution
  text: Mettre à jour l'adresse où le serveur d'exécution **{name}** peut être atteint.
  ...
  success: L'adresse du serveur d'exécution **{name}** a été mise à jour avec succès.
de:
  title: Ausführungsserver-Adresse ändern
  text: Aktualisieren Sie die Adresse, unter der der Ausführungsserver **{name}** erreichbar ist.
  ...
  success: Die Adresse des Ausführungsservers **{name}** wurde erfolgreich aktualisiert.
es:
  title: Cambiar dirección del servidor de ejecución
  text: Actualizar la dirección donde se puede alcanzar al servidor de ejecución **{name}**.
  ...
  success: Dirección del servidor de ejecución **{name}** actualizada con éxito.
zh:
  title: 更改执行服务器地址
  text: 更新执行服务器 **{name}** 可访问的地址。
  addressLabel: 执行服务器地址
  addressPlaceholder: 输入新的执行服务器地址
  addressHint: 执行服务器可访问的地址（例如：http://nano-runner-eu-west.acme.com）
  cancel: 取消
  confirm: 更改地址
  success: 成功更新执行服务器 **{name}** 的地址。
</i18n>
