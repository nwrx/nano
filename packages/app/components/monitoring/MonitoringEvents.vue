...
import type { MonitoringFlowThreadEventObject, MonitoringFlowThreadNodeEventObject } from '@nwrx/api'
...
const allEvents = computed(() => [
  ...
  .filter((x) => {
    if (eventTypes.value.length > 0 && !eventTypes.value.includes(x.type)) return false
    if (eventNames.value.length > 0 && !eventNames.value.includes(x.event)) return false
    return true
  })
  ...
  <div class="flex flex-col items-stretch h-full">
    ...
    <div class="px-md py-sm bg-app b-b b-app">
      <div class="flex items-center">
        <MonitoringToggle v-model="eventTypes" value="thread" icon="i-carbon:flow" />
        <MonitoringToggle v-model="eventTypes" value="node" icon="i-carbon:box" />
        <MonitoringToggle v-model="eventNames" value="start" icon="i-carbon:play-outline" />
        <MonitoringToggle v-model="eventNames" value="end" icon="i-carbon:checkmark-outline" />
        <MonitoringToggle v-model="eventNames" value="error" icon="i-carbon:error-outline" />
      </div>
    </div>
