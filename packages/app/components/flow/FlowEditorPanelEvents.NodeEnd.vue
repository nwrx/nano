...
<template>
  ...
  <FlowEditorPanelDataContainer>
    <FlowEditorPanelData :name="t('meta.event')" :modelValue="event.event" />
    <FlowEditorPanelData :name="t('meta.delta')" :modelValue="t('meta.delta.value', { delta: event.delta })" />
    <FlowEditorPanelData :name="t('meta.duration')" :modelValue="t('meta.duration.value', { duration: event.duration })" />
    <FlowEditorPanelData :name="t('meta.timestamp')" :modelValue="formatDateTime(event.timestamp)" />
    <FlowEditorPanelData :name="t('meta.id')" :modelValue="event.id" />
    <FlowEditorPanelData :name="t('meta.threadId')" :modelValue="event.threadId" />
    <FlowEditorPanelData :name="t('meta.executionId')" :modelValue="event.executionId" />
  </FlowEditorPanelDataContainer>
  ...
  <FlowEditorPanelDataContainer :title="t('data.title')">
    <template v-if="Object.keys(event.data).length > 0">
      <FlowEditorPanelData
        ...
        :modelValue="event.data[socket.key]"
        :socket="socket"
        :node="node"
      />
    </template>
    <template v-else>
      <div class="text-subtle p-sm">{{ t('data.empty') }}</div>
    </template>
  </FlowEditorPanelDataContainer>
  ...
  <FlowEditorPanelDataContainer :title="t('result.title')">
    <template v-if="Object.keys(event.result).length > 0">
      <FlowEditorPanelData
        ...
        :modelValue="event.result[socket.key]"
        :socket="socket"
        :node="node"
      />
    </template>
    <template v-else>
      <div class="text-subtle p-sm">{{ t('result.empty') }}</div>
    </template>
  </FlowEditorPanelDataContainer>
</template>
